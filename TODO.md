...
| 3 | **Item graphics corrupted** (hotbar, inventory, held/equipped item) | ⚠️ | GUI: `RenderPassMixins.radiance$onDrawIndexed` (per-draw geometry capture) + item atlas; held item: `EntityProxy.queueHandRebuild` | Items in the hotbar/inventory and the equipped/held item render corrupted. Hotbar/inventory items go through the overlay **draw-intercept** (a different path from the mob-texture fix), so suspect item-model **geometry/vertex-format/stride/index** capture in `radiance$onDrawIndexed`, or the item-model atlas — not texture selection. The held item goes through `queueHandRebuild` → `StorageVertexConsumerProvider` (may have partly improved with the #2 Sampler0 fix; re-check). Distinguish GUI-flat-item corruption from held-3D-item corruption when investigating. |
