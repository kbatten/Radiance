...
public class ModuleAttributeScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        context.text(font, module.translateText(module.name), 10, HEADER_HEIGHT + 8, 0xFFEAEAEA, true);

        if (rows.isEmpty()) {
            context.text(font, MODULE_ATTRIBUTE_SCREEN_NO_ATTRIBUTES, 10, 60, 0xFFB0B0B0, true);
            return;
        }
        ...
        for (int i = 0; i < rows.size(); i++) {
            ...
            if (visible) {
                context.text(font, module.translateText(row.cfg.name), ROW_LEFT, y + 6,
                    0xFFD0D0D0, true);
            }
