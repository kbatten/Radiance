...
final class AttributeWidgetUtil {
    ...
    static List<AbstractWidget> buildWidgets(Module module, AttributeConfig cfg, Font font,
        int width,
        ...
        return switch (type) {
            ...
            case "int" -> List.of(buildIntWidget(cfg, font, width));
            case "float" -> List.of(buildFloatWidget(cfg, font, width));
            case "string" -> List.of(buildStringWidget(cfg, font, width));
            case "vec3" -> buildVec3Widget(cfg, font, vec3ComponentWidth);
            default -> List.of(buildStringWidget(cfg, font, width));
        };
    }
    ...
    private static AbstractWidget buildIntWidget(AttributeConfig cfg, Font font,
        int width) {
        EditBox tf = new EditBox(font, 0, 0, width, 20, Component.empty());
        tf.setMaxLength(64);
    ...
    private static AbstractWidget buildFloatWidget(AttributeConfig cfg, Font font,
        int width) {
        EditBox tf = new EditBox(font, 0, 0, width, 20, Component.empty());
        tf.setMaxLength(64);
        tf.setValue(cfg.value == null ? "" : cfg.value);
    ...
    private static AbstractWidget buildStringWidget(AttributeConfig cfg, Font font,
        int width) {
        EditBox tf = new EditBox(font, 0, 0, width, 20, Component.empty());
        tf.setMaxLength(128);
        tf.setValue(cfg.value == null ? "" : cfg.value);
    ...
    private static List<AbstractWidget> buildVec3Widget(AttributeConfig cfg,
        ...
        EditBox x = vecField(font, v[0], componentWidth);
        EditBox y = vecField(font, v[1], componentWidth);
        EditBox z = vecField(font, v[2], componentWidth);
    ...
    private static EditBox vecField(Font font, float v, int width) {
        EditBox tf = new EditBox(font, 0, 0, width, 20, Component.empty());
        tf.setMaxLength(32);
